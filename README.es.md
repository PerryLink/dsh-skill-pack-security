...
| Superficie | Estado |
|---|---|
| Harness | DeepSeek Harness **`dsh-v0.1.5-alpha.1`** (GitHub tag, verificado el 2026-09-09: cadena completa de gates + smoke de instalación de perfil). Línea de dependencia npm: `@deepseek-ai/dsh` **0.1.2-rc.1** (peers `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0`). El sobre de sesión conserva su campo ignorable solo para compatibilidad de lectura de logs almacenados - Session.append aún no puede estamparlo, por lo que el comportamiento de la puerta no cambia. |
...
| `vet.userAgent` | `dsh-skill-pack-security/2.2.10 (+https://github.com/PerryLink/dsh-skill-pack-security)` | User-agent de descarga |
