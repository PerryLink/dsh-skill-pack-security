...
| Superfície | Status |
|---|---|
| Harness | DeepSeek Harness **`dsh-v0.1.3-alpha.1`** (GitHub tag, verificado em 2026-09-06: cadeia completa de gates + smoke de instalação de perfil). Linha de dependência npm: `@deepseek-ai/dsh` **0.1.2-rc.1** (peers `>=0.1.2-rc.1 <0.2.0`). O envelope de sessão mantém seu campo ignorable apenas para compatibilidade de leitura de logs armazenados - o Session.append ainda não consegue estampá-lo, então o comportamento da porta não muda. |
...
| `vet.userAgent` | `dsh-skill-pack-security/2.2.10 (+https://github.com/PerryLink/dsh-skill-pack-security)` | User-agent de download |
