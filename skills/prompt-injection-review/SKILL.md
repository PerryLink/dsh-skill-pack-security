...
运行时输入（git 命令枚举不到，必须列入检查表）：`web_search`/`web_fetch` 返回、MCP 服务返回、PR/issue 正文、克隆仓库里的 README/AGENTS.md、提交信息与分支名。
...
```sh
grep -rnE '^\s*(name|description|whenToUse):' .agents/skills/*/SKILL.md
```
